...
PlayerInit initializePlayerConnection() {
  PlayerInit init;
  if (gameState->numPlayersConnected >= MAX_PLAYERS) {
    init.validToJoin = 0b0;
    return init;
  } else {
    init.validToJoin = 0b1;
    init.playerId = gameState->numPlayersConnected - 1;
  }

  Vector2 centerWindow = (Vector2){WINDOW_WIDTH / 2.0f, WINDOW_HEIGHT / 2.0f};
  Vector2 connectingPlayerStartingPositions[5] = {
      Vector2Add(centerWindow, (Vector2){-10, 8}),
      Vector2Add(centerWindow, (Vector2){-8, -6}),
      Vector2Add(centerWindow, (Vector2){8, -6}),
      Vector2Add(centerWindow, (Vector2){10, 8}),
      Vector2Add(centerWindow, (Vector2){0, 8}),
  };
  ...
  init.startPos = connectingPlayerStartingPositions[init.playerId];
  init.playerColor = playerColors[init.playerId];
...
void *handleClientConnection(void *args) {
  ...
  // ready to now accept input from the client, recvs data of type playerInput
...
int serverAccept(const char *host, int port) {
  ...
  while (1) {
    ...
    if (gameState->state == GAME_ENDING) {
      // handle tear down logic here
      continue;
    }
    ...
    ClientConnectionData *clientData = calloc(1, sizeof(ClientConnectionData));
    if (clientData == NULL) {
